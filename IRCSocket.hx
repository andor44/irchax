...
class IRCSocket
{
	...
	public function start() : Void
	{
		...
		sock.output.writeString("NICK " + myself.nick + ENDL);
		sock.output.writeString("USER " + myself.nick + " " + Std.string(modebits) + " " + myself.nick + " :" + myself.realname + ENDL); // RFC says the server and host params are unimportant (in client mode?)
		isConnected = true;
	}
		...
		switch (getAction(line))
		{
			...
			case "JOIN":
				if (onJoin != null)
				{
					...
					onJoin(new IRCUser(user.split("!")[0], user.split("!")[1].split("@")[0], "", user.split("@")[1]), channel); // BUG: throws Segmentation fault originating from external C function on random, don't know what's causing it
				}
			case "QUIT":
				if (onQuit != null)
				{
